...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
    ...
    adcINFO.Voltage = (float)adcINFO.VbatOrg * 3.3f / 4096.0f * 48.0f+3 ; //11.0为实际电路中电阻分压比1:10之和
...
void StartScatteredTask(void const * argument)
{
  ...
  if (MPU6050ReadID() == 1)
	{
		...
		DISplay_ShowString(0,5,"Yrocker:",12);                          //字符显示
		DISplay_ShowString(0,6,"voltage:",12);													//字符显示
		DISplay_ShowString(0,7,"sensor is ready",12);										//字符显示
	}
	else
	{	DISplay_ShowString(0,0,"sensor not detected",12);}
  ...
  { 
    ...
    DISplay_ShowFolatNum(80,1,Mpu.deg_s.y+0.95,5,12);                  //y轴角速度显示
    ...
    DISplay_ShowFolatNum(80,4,encoderINFO.mainNumberValue,5,12);  //编码速度值
		DISplay_ShowNum(80,5,BluetoothParseMsg.Yrocker-2,5,12);  			//y摇杆值
    DISplay_ShowFolatNum(80,6,adcINFO.Voltage,5,12);           		//电压值	
    osDelay(40);
